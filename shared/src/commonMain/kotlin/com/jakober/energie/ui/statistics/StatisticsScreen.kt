...
/** Alles, was die Statistik zeigt; die Plattform sammelt es aus ihren Datenfluessen. */
data class StatisticsData(
    val range: Range,
    val date: LocalDate,
    val isToday: Boolean,
    val todayDate: LocalDate,
    val day: DayStatistics?,
    val rangeStats: RangeStatistics?,
    val settings: Settings,
    val lifetime: EnergyTotals?,
    val sessions: List<ChargeSession>,
    val currentMonth: RangeStatistics?,
    val storedDays: Int,
    val driving: List<DriveDay>,
    val daySamples: List<EnergySample>,
    val upgrade: UpgradeResult?,
    val upgradeKwh: Double,
...
class StatisticsActions(
    val setRange: (Range) -> Unit = {},
    val shift: (Int) -> Unit = {},
    val goToday: () -> Unit = {},
    val setUpgradeModuleKwh: (Double) -> Unit = {},
    val setUpgradeCost: (Double) -> Unit = {},
)

@Composable
fun StatisticsContent(data: StatisticsData, actions: StatisticsActions, contentPadding: PaddingValues) {
    val range = data.range
    val date = data.date
    val day = data.day
    val rangeStats = data.rangeStats
    val settings = data.settings
    val lifetime = data.lifetime
    val sessions = data.sessions
    val currentMonth = data.currentMonth
    val storedDays = data.storedDays
    val driving = data.driving
    val daySamples = data.daySamples
    val upgrade = data.upgrade
    val upgradeKwh = data.upgradeKwh
    val upgradeCost = data.upgradeCost
    ...
    val right: LazyListScope.() -> Unit = {
        ...
        val today = data.isToday || (range == Range.MONTH && Range.bounds(date, Range.MONTH) == Range.bounds(data.todayDate, Range.MONTH))
        ...
        if (storedDays >= 2) {
            item {
                StorageUpgradeCard(
                    ...
                    onModuleKwh = { actions.setUpgradeModuleKwh(it) }, onCost = { actions.setUpgradeCost(it) },
                )
            }
        }
        ...
        if (plugDays.any { it.plugs.isNotEmpty() }) {
            item { PlugsStatsCard(plugDays, settings) }
        }
